    ...
    <div className="animate-pulse space-y-2">
      {Array.from({ length: 6 }).map((_, i) => (
        <div key={i} className="grid" style={{ gridTemplateColumns: '100px repeat(5, 1fr)', gap: '4px', minHeight: '80px' }}>
          <div className="rounded-2xl p-2 glass" style={{ opacity: 0.3 }} />
          {Array.from({ length: 5 }).map((_, j) => (
            <div key={j} className="rounded-2xl p-2 glass" style={{ opacity: 0.2 }} />
          ))}
        </div>
      ))}
    </div>
...
export function SkeletonCard() {
  return (
    <div className="animate-pulse rounded-2xl p-4 glass">
